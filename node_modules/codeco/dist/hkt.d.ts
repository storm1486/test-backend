 ...
 */
export type Kind<F extends HKT, A> = F extends HKT ? (F & {
    readonly _A: A;
})["type"] : never;
/**
 * Map values of Dictionary.
 */
type MapIn<P, F extends HKT> = {
    [K in keyof P]: Kind<F, P[K]>;
};
export type MapOf<Tuple extends readonly unknown[], F extends HKT, Result extends List = []> = Tuple extends readonly [
    unknown,
    ...unknown[]
] ? MapOf<Tail<Tuple>, F, Append<Result, Kind<F, Head<Tuple>>>> : Result;
...
export type Intersection<Tuple extends readonly unknown[], Result = {}> = Tuple extends readonly [unknown, ...unknown[]] ? Intersection<Tail<Tuple>, Head<Tuple> & Result> : Result;
export {};
