...
program
    ...
    .option('--ethereum-rpc <url>', 'The Ethereum RPC URL used for communicating with Ethereum blockchain. Deprecated.')
    ...
    .option('--state-store-directory <string>', `The directory path used for state store. Defaults to HOME_DIR/.ceramic/statestore. Deprecated.`)
    .option('--state-store-s3-bucket <string>', `The S3 bucket name to use for state store. If not provided data will only be saved locally but not to S3. Deprecated.`)
    ...
    .option('--log-directory <dir>', 'Store logs in this directory. Defaults to HOME_DIR/.ceramic/logs. Deprecated')
    ...
    .option('--cors-allowed-origins <list>', 'Space-separated list of strings and/or regex expressions to set for Access-Control-Allow-Origin . Defaults to all: ".*". Deprecated.')
    .option('--sync-override <string>', 'Global forced mode for syncing all streams. One of: "prefer-cache", "sync-always", or "never-sync". Defaults to "prefer-cache". Deprecated.')
    .option('--disable-composedb', 'Run node without ComposeDB indexing enabled.')
    .option('--read-only', 'Disables http endpoints that perform writes, making the node effective for reading data from the network but not for writing to it.  It is disabled by default')
    .description('Start the daemon')
    .action(async ({ config, ipfsApi, ethereumRpc, anchorServiceApi, ipfsPinningEndpoint, stateStoreDirectory, stateStoreS3Bucket, port, hostname, debug, verbose, logToFiles, logDirectory, metricsExporterEnabled, collectorHost, network, pubsubTopic, corsAllowedOrigins, syncOverride, disableComposedb, readOnly, }) => {
    await CeramicCliUtils.createDaemon(config, ipfsApi, ethereumRpc, anchorServiceApi, ipfsPinningEndpoint, stateStoreDirectory, stateStoreS3Bucket, port, hostname, debug, verbose, logToFiles, logDirectory, metricsExporterEnabled, collectorHost, network, pubsubTopic, corsAllowedOrigins, syncOverride, disableComposedb, readOnly).catch((err) => {
...
program
    ...
    .option('--deterministic', 'Document content is created deterministically from the inputs.  This means ' +
...
schemas
    ...
    .option('--only-genesis', 'Only create the genesis object. No anchor will be created')
    .option('--controllers <controllers>', 'Specify a comma-separated list of the controllers of the schema document. Defaults to' +
    ' current user')
    .option('--deterministic', 'Document content is created deterministically from the inputs.  This means ' +
