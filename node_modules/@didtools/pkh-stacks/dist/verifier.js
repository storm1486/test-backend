import { SiwStacksMessage, verifyTimeChecks, assertSigned } from '@didtools/cacao';
import { AccountId } from 'caip';
import { verifyMessageSignatureRsv, hashMessage } from '@stacks/encryption';
import { publicKeyFromSignatureRsv, createMessageSignature, getAddressFromPublicKey } from '@stacks/transactions';
import { bytesToHex } from '@stacks/common';
export function getStacksVerifier() {
    return {
        // eslint-disable-next-line @typescript-eslint/require-await
        'stacks:secp256k1': async (cacao, opts)=>{
            verifyStacksSignature(cacao, opts);
        }
    };
}
export function verifyStacksSignature(cacao, options) {
    assertSigned(cacao);
    ...
    const recoveredPublicKey = publicKeyFromSignatureRsv(bytesToHex(hashMessage(message)), createMessageSignature(signature));
