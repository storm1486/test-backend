...
export const GraphQLDate = new GraphQLScalarType({
    ...
    serialize: (dateString)=>{
        if (typeof dateString === 'string' && validateDate(dateString)) {
            return dateString;
        } else throw new TypeError(`Date cannot represent an invalid date-string ${String(dateString)}.`);
    },
    parseValue: (value)=>{
    ...
    serialize: (dateString)=>{
        if (typeof dateString === 'string' && validateDateTime(dateString)) {
            if (dateString.indexOf('Z') !== -1) {
                return dateString;
            } else throw new TypeError(`DateTime string must be formatted to UTC time ${String(dateString)}.`);
        } else throw new TypeError(`DateTime cannot represent an invalid date-time-string ${String(dateString)}.`);
    },
    parseValue: (value)=>{
        if (typeof value === 'string' && validateDateTime(value)) {
            return value;
        } else throw new TypeError(`DateTime cannot represent an invalid date-time-string ${String(value)}.`);
    },
    parseLiteral: (ast)=>{
        if (ast.kind !== Kind.STRING) {
